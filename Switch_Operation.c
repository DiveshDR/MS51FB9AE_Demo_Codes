...
void main(void) {
    ...
    // Configure P0.7 as ADC input (ADC channel 2)
    AINDIDS |= (1 << 7); // Enable P0.7 as analog input
    P0M1 |= (1 << 7);    // Disable digital input/output (set to high-impedance)
    P0M2 &= ~(1 << 7);

    // Enable ADC and configure
    ADCCON1 |= 0x01;    // Enable ADC (bit 0)
    ADCCON0 &= ~0x0F;   // Clear channel bits
    ADCCON0 |= 0x02;    // Select channel 2 (P0.7)

    while (1) {
        unsigned int adc_value; // Declare at start of block

        // Start ADC conversion
        ADCCON0 |= (1 << 6); // Set ADCS (start conversion)

        // Wait for conversion complete
        while (!(ADCCON0 & (1 << 7))); // Check ADF flag
        ...
        // Determine what to display based on ADC value
        if (adc_value >= 123 && adc_value <= 164) { // 0.6V to 0.8V
            display[0] = '1';
            display[1] = '1';
            display[2] = '1';
        } else if (adc_value >= 266 && adc_value <= 327) { // 1.3V to 1.6V
            display[0] = '2';
            display[1] = '2';
            display[2] = '2';
        } else if (adc_value >= 491 && adc_value <= 532) { // 2.4V to 2.6V
            display[0] = '3';
            display[1] = '3';
            display[2] = '3';
        } else if (adc_value >= 142 && adc_value <= 144) { // ~0.7V (range for stability)
            display[0] = '4';
            display[1] = '4';
            display[2] = '4';
        } else {
